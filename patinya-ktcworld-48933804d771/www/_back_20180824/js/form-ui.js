...
$(document).ready(function() {
    ...
    $('.form-flight input').change(function(){
        ...
        if($('.form-flight .input-origin').hasClass('filled') && $('.form-flight .input-destination').hasClass('filled')){
    ...
    $('.form-flight .swap-btn').click(function(e){
        e.preventDefault();
        var ori = $('.form-flight .input-origin input').val();
        var des = $('.form-flight .input-destination input').val();
        $('.form-flight .input-origin input').val(des);
        $('.form-flight .input-destination input').val(ori);
    });
    ...
    $('.form-flight .input-select-seat').autocomplete({
        ...
        minLength: 0,
        position: { my: "left+0 top+20" },
        classes: { 'ui-autocomplete': 'form-bubble' },
        select: function(e, ui) {
            e.preventDefault();
            $(this).val(ui.item.label).trigger('change');
        },
    ...
    $('.form-flight .input-date-picker').datepicker({
        ...
        beforeShow: function() {
            ...
            setTimeout(function () {
                $('#ui-datepicker-div').append('<p class="warning">หากต้องการทำการจองในวันดังกล่าว กรุณาติดต่อ KTC World Travel Service โทร. 02 123 5050</p>');
            }, 80);
        },
